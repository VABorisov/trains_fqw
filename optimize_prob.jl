...
function main()
    ...
    for i in 1:size(S_div, 1)
        set_start_value(x[i], 1.1 * (1 / V_const))
    end
    
    function create_objective_closure(
        S_div, L_common, L1, q_arr, f_arr, ae_arr, gamma_arr, theta1, theta2, theta3,
        a1, a2, a3, p_s_l_line, p_s_l_wave, p_s_l_cover, P_s, part_with_gamma_theta_1,
        part_with_gamma_theta_2, part_with_gamma_theta_3, w, mu, sum_d_arr
    )
        function objective_closure(x::T...) where {T <: Real}
            x_vector = collect(x)
            result = ProbabilisticRiskFunction(
                x_vector, S_div, L_common, L1, q_arr, f_arr, ae_arr, gamma_arr,
                theta1, theta2, theta3, a1, a2, a3, p_s_l_line, p_s_l_wave,
                p_s_l_cover, P_s, part_with_gamma_theta_1, part_with_gamma_theta_2,
