...
function precalculation(S, S_div, theta1, theta2, theta3, muliplier_p_line_s_l, muliplier_p_wave_s_l, muliplier_p_cover_s_l, ae_s, gamma_s, L_common, sum_d_arr, hi_s, b1, b2, mu)
...
function M_C(f, v, step, p_type, p1_arr, p2_arr)
	...
	# common_part = 1 - (-(9 / 200) * v + 1) * exp(-0.005 * (v ^ 2))
...
function main() 
    ...
    ae_arr, gamma_arr, hi_arr, p_s_l_line, p_s_l_wave, p_s_l_cover, part_with_gamma_theta_1, part_with_gamma_theta_2, part_with_gamma_theta_3, p1_arr, p2_arr = precalculation(
    S, S_div,theta1,theta2, theta3, muliplier_p_line_s_l, muliplier_p_wave_s_l, muliplier_p_cover_s_l, ae_s, gamma_s,  L_common, sum_d_arr, hi_s, b1, b2, mu)
    v_arrs = [v5s, v6s, v7s, v8s, v9s, v10s, v11s, v12s, v13s, v14s, v15s, v16s, v17s, v18s, v19s, v20s]
    results = zeros(length(v_arrs),1)
    for i = 1:size(results,1)
    	results[i] = Risks(S_div, L_common, L1, q_arr, f_arr, step_arr, v_arrs[i], ae_arr, gamma_arr, hi_arr, theta1, theta2, theta3, a1, a2, a3, p1_arr, p2_arr, p_s_l_line, p_s_l_wave, p_s_l_cover, P_s, part_with_gamma_theta_1, part_with_gamma_theta_2, part_with_gamma_theta_3, w, mu, sum_d_arr)    end
    println(results)
end
