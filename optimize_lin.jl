...
function main()
   ...
   new_results = zeros(length(result))
   a = 0.1
   h = 1
   for i = 1:size(result, 1) 
		new_results[i] = sqrt((result[i] ^ 2) + 2 * a * h)
   end
   println(new_results)
   r2 = LinearRiskFunction(1 ./ new_results, S_div, L_common, L1, q_arr, f_arr, step_arr, ae_arr, gamma_arr, hi_arr, theta1, theta2, theta3, a1, a2, a3, p1_arr, p2_arr, p_s_l_line, p_s_l_wave, p_s_l_cover, P_s, part_with_gamma_theta_1, part_with_gamma_theta_2, part_with_gamma_theta_3, w, mu, sum_d_arr)
   println(r2)
end
